...
//
// Wat doet dit?
// Leest het Excel-bestand "Urenrapportage" uit (dat een manager handmatig
// heeft geüpload via de knop op het dashboard — zie hieronder) en berekent
// per maand (en per maand+opdrachtgever) de declarabiliteit, ziekte-uren,
// marge en overuren. De uitkomsten worden weggeschreven naar de tabellen
// 'uren_kpi_maand' en 'uren_kpi_opdrachtgever_maand', die het dashboard
// vervolgens gewoon uitleest zoals elke andere tabel.
//
// Waar komt het bestand vandaan?
// Eerder probeerden we het bestand automatisch op te halen vanaf een
// SharePoint-deellink, maar SharePoint bleek anonieme (niet-ingelogde)
// downloads regelmatig te blokkeren (HTTP 403), ook als de link zelf goed
// was ingesteld — waarschijnlijk door een instelling op organisatieniveau.
// Daarom is dit nu omgezet naar een handmatige import: een manager klikt op
// het dashboard op "🔄 Vernieuwen vanuit Urenrapportage", kiest het
// Urenrapportage.xlsx-bestand van zijn eigen computer, en het dashboard
// uploadt dat rechtstreeks naar de privé Supabase Storage-bucket
// 'urenrapportage' (zie urenrapportage_opslag.sql voor het aanmaken van die
// bucket en de bijbehorende rechten). Deze functie haalt het bestand
// vervolgens op uit die bucket via een kortstondige ondertekende URL —
// geen SharePoint, geen externe afhankelijkheid meer.
//
...
// pg_cron (zie urenrapportage_cron.sql) — dat herverwerkt gewoon opnieuw het
// laatst geüploade bestand (geen nieuwe data, maar ook geen kwaad). Wordt
// vooral handmatig gestart via de importknop op het dashboard (alleen door
// een manager), die na het uploaden deze functie een paar keer achter elkaar
// aanroept totdat alle porties klaar zijn.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Wordt alleen door het dashboard meegestuurd bij de éérste aanroep na
    // een nieuwe handmatige upload — dwingt een verse, volledige herverwerking
    // af, ook als er vandaag al een keer gesynchroniseerd is.
    const body = await req.json().catch(() => ({} as Record<string, unknown>));
    const forceerNieuw = (body as { forceerNieuw?: boolean })?.forceerNieuw === true;

    // Bestand opzoeken in de Storage-bucket 'urenrapportage' (daar zet de
    // importknop op het dashboard het neer) en een kortstondige ondertekende
    // URL ervoor maken — alleen deze functie (met de service-role key) kan
    // dat, de bucket zelf is privé.
    const { data: signed, error: signErr } = await admin
      .storage
      .from("urenrapportage")
      .createSignedUrl("Urenrapportage.xlsx", 120);
    if (signErr || !signed?.signedUrl) {
      return json({ error: "Geen Urenrapportage-bestand gevonden. Klik op '🔄 Vernieuwen vanuit Urenrapportage' en kies eerst het Excel-bestand van je computer." }, 400);
    }
    const bestandUrl = signed.signedUrl;
    ...
    if (!forceerNieuw && status.fase === "klaar" && laatsteDag === vandaag) {
    ...
    if (forceerNieuw || status.fase !== "verwerken") {
      // Nieuwe run starten (nachtelijk, of na een verse handmatige upload).
      deelHuidig = 0;
    ...
    const { rows, shared, regelsGezien } = await streamPass(bestandUrl, deelHuidig);
